...
# --- Time series ---
for csv in sorted(DIR.glob("*_transport_advection_plume_2d.csv")):
...
# --- 2D spatial ---
for csv in sorted(DIR.glob("*_transport_advection_plume_2d_spatial.csv")):
    data = np.genfromtxt(csv, delimiter=",", names=True)
    xs = np.unique(data["x"])
    ys = np.unique(data["y"])
    nx, ny = len(xs), len(ys)
    # CSV is written row-major (y outer, x inner)
    Z = data["talbot"].reshape(ny, nx)
    X, Y = np.meshgrid(xs, ys)

    fig, ax = plt.subplots(figsize=(9, 4.5), constrained_layout=True)
    cf = ax.contourf(X, Y, Z, levels=30)
    fig.colorbar(cf, ax=ax, label="Concentration [kg/m³]")
    ax.plot(0, 0, "wx", label="Release point")
    ax.set_xlabel("x [m]")
    ax.set_ylabel("y [m]")
    ax.set_title(r"$C(x,y,t=5\,\mathrm{s})$")
    ax.set_aspect("equal")
